...
(function($) {
  ...
  $.fn.resetAutosize = function(options){
	...
	$.fn.getAllTags = function() {
		...
		// get the id
		var id = $(this).attr('id');
		//console.info("in getAllTags: ");

		var t = [];

		// Get each tag value and add it to the array
	...
	/**
	 * set tag id
	 * Find the tag with the given id and set its id
	 */ 
	$.fn.setId = function(tagVal, newId) {

		// get the id
		var id = $(this).attr('id');

		$("#" + id + "_tagsinput > .tag[data-title=\"" + tagVal + "\"]").attr("id", newId);
	};
			...
			if (settings.interactive) { 
				...
				data.removeWithBackspace && $(data.fake_input).bind('keydown', function(event)
				{
					if(event.keyCode == 8 && $(this).val() == '')
					{
						 ...
						 // get the value of the last tag
						 var last_tag = $(this).closest('.tagsinput').find('.tag:last').text();

						 // get the id of the container
						 var id = $(this).attr('id').replace(/_tag$/, '');

						 // replace white space in the names
